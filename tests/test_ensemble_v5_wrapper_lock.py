"""Regression tests for the E6-v5 wrapper's persistent execution lock."""
...
def _sandboxed_wrapper(
    ...
    script.chmod(0o700)
    lock = run_root / "runtime" / "v5" / "ensemble_v5.lock"
...
def test_parent_replacement_between_execs_is_rejected(tmp_path: Path):
    runtime = tmp_path / "run" / "runtime" / "v5"
    moved = tmp_path / "run" / "runtime" / "v5-original"
